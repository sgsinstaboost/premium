...
window.safeHideLoader = function() {
...
onAuthStateChanged(auth, async (user) => {
    try {
        ...
        console.error("Auth state catch error:", authErr);
    } finally {
        window.hideGlobalLoadingScreen();
    }
});
...
window.showPolicy = function(type) {
    ...
    if(type === 'privacy') {
        ...
        contentEl.innerHTML = `
            <div class="space-y-4">
                <p class="text-slate-400"><strong>Effective Date:</strong> July 24, 2026 | <strong>Last Updated:</strong> July 24, 2026</p>
                <p>Welcome to <strong>SGS InstaBoost</strong>. Please read these Terms of Agreement ("Terms", "Agreement") carefully before using our mobile application.</p>
            </div>`;
    }
    modal.classList.remove('hidden');
    modal.classList.add('flex');
